...
\begin{table}[ht]\label{table:spinbutton-params}
    \begin{ctucolortab}
        ...
        \end{tabularx}
    \end{ctucolortab}
    \caption{Parametry primitivní funkce createSpinButton}
\end{table}

Property \texttt{values} je pole hodnot generického typu, který ale musí být typu \texttt{number}, nebo \texttt{string}.
Je možné, že hodnoty ve \texttt{values} nemají sémantický význam, proto jejich význam je možné předat v property \texttt{mapping}.
...
\begin{table}[ht]
    \begin{ctucolortab}
        ...
        \end{tabularx}
    \end{ctucolortab}
    \caption{ARIA atributy funkce createSpinButton}
...
Dvě důležité vlastnosti \texttt{aria-valuemin}\footnote{\url{https://w3c.github.io/aria/\#aria-valuemin}} a \texttt{aria-valuemax}\footnote{\url{https://w3c.github.io/aria/\#aria-valuemax}} definují minimální, respektive maximální hodnotu, protože se jedná o diskrétní výběr hodnot z intervalu.
...
Parametry \texttt{aria-labelledby}\footnote{\url{https://www.w3.org/TR/wai-aria/\#aria-labelledby}} a \texttt{aria-label}\footnote{\url{https://www.w3.org/TR/wai-aria/\#aria-label}} slouží pro pojmenování instance komponenty pro splnění požadavku \href{ofr13}{\fr{OFR 1.3}}.
...
\begin{table}[ht]\label{table:toolbar-aria}
    \begin{ctucolortab}
        ...
        \end{tabularx}
    \end{ctucolortab}
    \caption{ARIA atributy funkce createToolbar}
\end{table}
